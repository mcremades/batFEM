...
cases = {
    "1": {"folder": f"test", "style": "-", "marker":"", "color": "black"}
}

reference_case = "1"
target_times = np.linspace(0, 8000, 6)
...
plot_spatial_field("j_sei", r"$V\cdot j_{sei}$ [A]",factors=[V_a,1,V_c])
plot_spatial_field("j_lpl", r"$V\cdot j_{lpl}$ [A]",factors=[V_a,1,V_c])

plot_spatial_field("eta", r"$\eta_{int}$ [A]",factors=[1,1,1])
plot_spatial_field("eta_sei", r"$\eta_{sei}$ [A]",factors=[1,1,1])
plot_spatial_field("eta_lpl", r"$\eta_{lpl}$ [A]",factors=[1,1,1])
plt.show()